...
public class CartServiceImpl implements CartService {
    ...
    //添加商品进入配置车的实现
    @Override
    public void addProfileCart(Integer uid, String username, Integer id) {
        Cart cart = new Cart();
        User resultUser = userMapper.findByUid(uid);
        if (resultUser == null) {
            throw new DataSelectException("用户信息不存在");
        }
        Product resultProduct = productMapper.findById(id);
        if (resultProduct == null) {
            throw new DataSelectException("商品信息不存在");
        }
        ...
        if (resultCart >=1 ) {
            throw new DataInsertException("该类商品已经加入购物车不要重复添加");
        }
    ...
    public void deleteProfileCart(Integer uid, Integer id) {
        User resultUser = userMapper.findByUid(uid);
        if (resultUser == null) {
            throw new DataSelectException("用户数据不存在");
        }
        Product resultProduct = productMapper.findById(id);
        if (resultProduct == null) {
            throw new DataSelectException("产品不存在");
        }
        Integer rows = cartMapper.deleteProfileCart(uid,id);
        if (rows != 1) {
            throw new DataDeleteException("删除数据失败");
        }
    }
